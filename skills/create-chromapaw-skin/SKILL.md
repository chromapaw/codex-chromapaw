...
5. Infer light, dark, or adaptive mode from the request. Ask only when the choice materially changes the desired result. Protect the normalized central reading safe zone and keep faces, logos, high-frequency texture, and bright highlights away from it. Independently classify (a) where the main subject is positioned and (b) whether it is `ambient`, `supporting`, or `showcase`. Do not classify every visible person, character, logo, or object as `showcase`; use that value only when the request or composition makes unobstructed subject display a primary goal.
...
7. Use the Codex workspace Python runtime with Pillow. Run `scripts/prepare_skin_request.py` with `--theme-profile`, the original image, and, when generated, the approved expanded artwork. Pass `--subject-placement left` or `right` for a side subject, `edge-balanced` for decorative subjects on both sides, and `source` when source positioning should be preserved. Also pass `--subject-display-priority ambient|supporting|showcase` and normally keep `--content-layout auto`. Auto keeps normal Codex conversation width for ambient/supporting subjects and reserves side space only for a left/right `showcase` subject. Use an explicit content-layout override only when the user requests it.
...
- Subject placement must not shrink the conversation by itself. Normal width is the default; permanent side reservation is allowed only for an intentionally unobstructed showcase subject or an explicit user override.
