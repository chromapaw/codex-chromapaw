...
ChromaPaw 0.4.6 has platform-neutral code paths for portable pet and contrast-safe skin assets. Pet generation additionally requires a compatible external `hatch-pet` workflow, and both generation paths still require real-Mac acceptance testing. The separate persistent launchers are Windows-only; live macOS skin activation is not implemented or claimed.
